...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`BLE tracker backend running on http://0.0.0.0:${PORT}`);
});
...
async function handleApi(req, res) {
  ...
  if (req.method === "GET" && url.pathname === "/api/health") {
    ...
    respondJson(res, 200, {
      ...
      sensorReadings: db.sensorReadings?.length ?? 0,
    });
    return;
  }

  if (req.method === "POST" && url.pathname === "/api/admin/summary") {
    const body = await readJson(req);
    if (!process.env.ADMIN_REGISTRATION_SECRET || body.adminSecret !== process.env.ADMIN_REGISTRATION_SECRET) {
      await withDb((db) => {
        logSecurityEvent(db, "ADMIN_SUMMARY_DENIED", { clientIp });
      }, { write: true });
    ...
    const db = await loadDb();
    respondJson(res, 200, buildAdminSummary(db));
    return;
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/bootstrap") {
    ...
    respondJson(res, 200, {
      ...
      sensorRules: db.sensorRules.filter((entry) => entry.userId === auth.user.id),
  ...
  if (req.method === "GET" && url.pathname === "/api/sensor-readings") {
    const db = await loadDb();
    const auth = requireAuth(db, req);
    const deviceId = url.searchParams.get("deviceId");
    const limit = Math.min(Math.max(Number(url.searchParams.get("limit") ?? 200), 1), 500);
    const readings = deviceId
      ? (db.sensorReadings ?? []).filter(
          (entry) =>
            entry.deviceId === normalizeDeviceId(deviceId) &&
            entry.ownerUserId === auth.user.id
        )
      : (db.sensorReadings ?? []).filter((entry) => entry.ownerUserId === auth.user.id);
    respondJson(res, 200, readings.slice(0, limit));
    return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/alerts/custom") {
    const body = await readJson(req);
    const result = await withDb((db) => {
      const auth = requireAuth(db, req);
      const normalizedDeviceId = normalizeDeviceId(body.deviceId);
      const device = db.devices.find((entry) => entry.deviceId === normalizedDeviceId);
      if (!device || device.ownerUserId !== auth.user.id) {
        const error = new Error("device not found or not owned by you");
        error.statusCode = 404;
        throw error;
      }
      
      const type = typeof body.type === "string" ? body.type.trim() : "CUSTOM_ALERT";
      const message = typeof body.message === "string" ? body.message.trim() : "Custom alert triggered.";
      
      const alert = createAlert(db, type, device, message, body.extra || {});
      return { alert };
    }, { write: true });
  ...
  if (req.method === "GET" && url.pathname === "/api/sensor-rules") {
    const db = await loadDb();
    const auth = requireAuth(db, req);
    respondJson(
      res,
      200,
      db.sensorRules.filter((entry) => entry.userId === auth.user.id)
    );
    return;
  }

  if (req.method === "POST" && url.pathname === "/api/sensor-rules") {
    const body = await readJson(req);
    const result = await withDb((db) => {
      const auth = requireAuth(db, req);
      const rule = {
        ...
        name: typeof body.name === "string" ? body.name.trim() : "Custom Rule",
        createdAt: new Date().toISOString(),
      };
      db.sensorRules.push(rule);
      return rule;
    }, { write: true });
    respondJson(res, 201, result);
    return;
  ...
  if (req.method === "DELETE" && url.pathname.startsWith("/api/sensor-rules/")) {
    const ruleId = url.pathname.split("/").pop();
    const removed = await withDb((db) => {
      const auth = requireAuth(db, req);
      const index = db.sensorRules.findIndex(
        (entry) => entry.id === ruleId && entry.userId === auth.user.id
      );
      if (index === -1) {
        const error = new Error("sensor rule not found");
        error.statusCode = 404;
        throw error;
      }
      const [deleted] = db.sensorRules.splice(index, 1);
      return deleted;
    }, { write: true });
    respondJson(res, 200, removed);
    return;
...
function buildAdminSummary(db) {
  const registrations = db.deviceRegistrations.map((entry) => ({
    id: entry.id,
    deviceId: entry.deviceId,
    note: entry.note ?? "",
    claimedByUserId: entry.claimedByUserId ?? null,
  ...
  return {
    ...
    health: {
      ...
      devices: db.devices.length,
      registrations: db.deviceRegistrations.length,
      claimedRegistrations: registrations.filter((entry) => entry.claimedByUserId).length,
      openAlerts: db.alerts.filter((entry) => entry.status === "open").length,
      sensorReadings: db.sensorReadings?.length ?? 0,
      scannerSightings: db.scannerSightings.length,
    ...
    devices: db.devices
      .map((device) => ({
        ...
        lastPacket: device.lastPacket,
        lastSensors: device.lastSensors ?? null,
        lastSensorSeenAt: device.lastSensorSeenAt ?? null,
        geofenceState: device.geofenceState,
        proximityMonitoringEnabled: device.proximityMonitoringEnabled,
      }))
      .sort((left, right) => String(right.lastSeenAt ?? "").localeCompare(String(left.lastSeenAt ?? "")))
      .slice(0, 100),
    recentSensorReadings: (db.sensorReadings ?? []).slice(0, 50),
    recentSecurityEvents: db.securityEvents.slice(0, 50),
  };
}
