...
const MAX_SENSOR_READING_RECORDS = 1000;
...
function normalizeSensors(sensors) {
  ...
  const temperatureC =
    sensors.temperatureC === null || sensors.temperatureC === undefined
      ? null
      : Number(sensors.temperatureC);
  const humidityRh =
    sensors.humidityRh === null || sensors.humidityRh === undefined
      ? null
      : Number(sensors.humidityRh);
  const lux = sensors.lux === null || sensors.lux === undefined ? null : Number(sensors.lux);
  const accelXMg = sensors.accelXMg === null || sensors.accelXMg === undefined ? null : Number(sensors.accelXMg);
  const accelYMg = sensors.accelYMg === null || sensors.accelYMg === undefined ? null : Number(sensors.accelYMg);
  const accelZMg = sensors.accelZMg === null || sensors.accelZMg === undefined ? null : Number(sensors.accelZMg);
  const gyroXDps = sensors.gyroXDps === null || sensors.gyroXDps === undefined ? null : Number(sensors.gyroXDps);
  const gyroYDps = sensors.gyroYDps === null || sensors.gyroYDps === undefined ? null : Number(sensors.gyroYDps);
  const gyroZDps = sensors.gyroZDps === null || sensors.gyroZDps === undefined ? null : Number(sensors.gyroZDps);
  const vibrationScore =
    sensors.vibrationScore === null || sensors.vibrationScore === undefined
      ? null
      : Number(sensors.vibrationScore);

  const normalized = {
    temperatureC: Number.isFinite(temperatureC) ? temperatureC : null,
    humidityRh: Number.isFinite(humidityRh) ? humidityRh : null,
    lux: Number.isFinite(lux) ? lux : null,
    accelXMg: Number.isFinite(accelXMg) ? accelXMg : null,
    accelYMg: Number.isFinite(accelYMg) ? accelYMg : null,
    accelZMg: Number.isFinite(accelZMg) ? accelZMg : null,
    gyroXDps: Number.isFinite(gyroXDps) ? gyroXDps : null,
    gyroYDps: Number.isFinite(gyroYDps) ? gyroYDps : null,
    gyroZDps: Number.isFinite(gyroZDps) ? gyroZDps : null,
    vibrationScore: Number.isFinite(vibrationScore) ? vibrationScore : null,
  };

  if (
    normalized.temperatureC === null &&
    normalized.humidityRh === null &&
    normalized.lux === null &&
    normalized.accelXMg === null &&
    normalized.accelYMg === null &&
    normalized.accelZMg === null &&
    normalized.gyroXDps === null &&
    normalized.gyroYDps === null &&
    normalized.gyroZDps === null &&
    normalized.vibrationScore === null
  ) {
    return null;
  }
 ...
 */
export function createAlert(db, type, device, message, extra = {}) {
...
export function recordSighting(db, payload) {
  ...
  const sensors = normalizeSensors(payload.sensors ?? payload.packet?.sensors ?? null);
  ...
  if (sensors) {
    device.lastSensors = sensors;
    device.lastSensorSeenAt = seenAt;
  }
  device.lastPacket = {
    ...packet,
  ...
  if (sensors) {
    ...
    db.sensorReadings.length = Math.min(db.sensorReadings.length, MAX_SENSOR_READING_RECORDS);
  }
...
export function evaluateRealtimeAlerts(db, device, previousState = null) {
  ...
  if (previousState !== null && previousState !== device.lastPacket.bagState) {
    closeAlert(db, "STATE_CHANGE_SIGNAL", device.deviceId);
    ...
    // Clear stale signal-only alerts once the tracker returns to a normal packet flow.
    closeAlert(db, "STATE_CHANGE_SIGNAL", device.deviceId);
  }
  ...
  // Evaluate Sensor Rules — must run BEFORE the geofence guard which may return early.
  if (device.lastSensors) {
    const ownerSensorRules = (db.sensorRules ?? []).filter((entry) => entry.userId === device.ownerUserId);
    for (const rule of ownerSensorRules) {
      let value = null;
      switch (rule.sensorType) {
        case "temperatureC": value = device.lastSensors.temperatureC; break;
        case "humidityRh": value = device.lastSensors.humidityRh; break;
        case "lux": value = device.lastSensors.lux; break;
        case "vibrationScore": value = device.lastSensors.vibrationScore; break;
        case "accelXMg": value = device.lastSensors.accelXMg; break;
        case "accelYMg": value = device.lastSensors.accelYMg; break;
        case "accelZMg": value = device.lastSensors.accelZMg; break;
        case "gyroXDps": value = device.lastSensors.gyroXDps; break;
        case "gyroYDps": value = device.lastSensors.gyroYDps; break;
        case "gyroZDps": value = device.lastSensors.gyroZDps; break;
      }

      if (value !== null && value !== undefined) {
        let triggered = false;
        if (rule.minValue !== null && value < rule.minValue) triggered = true;
        if (rule.maxValue !== null && value > rule.maxValue) triggered = true;

        if (triggered) {
          createAlert(
            db,
            "SENSOR_RULE_VIOLATION",
            device,
            `${rule.name}: ${rule.sensorType} reading ${value} is outside allowed range (${rule.minValue} – ${rule.maxValue}).`,
            { ruleId: rule.id }
          );
        }
      }
