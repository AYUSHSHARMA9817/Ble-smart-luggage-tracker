...
Only the **backend** needs to be hosted publicly. The Android app connects to the backend URL over HTTPS.
...
Before clicking **Apply**, find the `ADMIN_REGISTRATION_SECRET` variable and enter a long random string (e.g. `openssl rand -hex 32`). Keep this secret safe — you need it to pre-register tracker hardware.
...
```json
{"ok":true,"manufacturerId":"0xFF01","users":0,"devices":0,"geofences":0,"alerts":0,"registrations":0}
```

### Step 6 — Persistent disk

The `render.yaml` already requests a 1 GB persistent disk mounted at `/data`. The `DATA_PATH` variable is set to `/data/app.json`. All user data survives service restarts automatically.
...
4. Add environment variables: `PORT=8787`, `DATA_PATH=/data/app.json`, `ADMIN_REGISTRATION_SECRET=<secret>`.
...
- [ ] Persistent disk / volume is mounted and data survives restart
...
| `DATA_PATH` | `data/app.json` | Path to the JSON database file |
