...
import com.bletracker.app.data.SensorRuleDto
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _uiState = MutableStateFlow(
        MainUiState(
            ...
            googleWebClientId = prefs.googleWebClientId,
    ...
    fun saveSettings(
        ...
        prefs.googleWebClientId = googleWebClientId
        _uiState.update {
            it.copy(
                backendBaseUrl = prefs.backendBaseUrl,
                scannerAutostartEnabled = prefs.scannerAutostartEnabled,
    ...
    fun saveGoogleWebClientId(clientId: String) {
        prefs.googleWebClientId = clientId
        _uiState.update {
            it.copy(
                googleWebClientId = prefs.googleWebClientId,
            )
    ...
    fun signInWithGoogle(idToken: String, serverClientId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            runCatching {
                val auth = api.authGoogle(idToken, serverClientId)
                onAuthSuccess(auth.user.id, auth.authToken, auth.user, "Signed in with Google")
            }.onFailure { error ->
                _uiState.update { it.copy(statusMessage = error.message ?: "Google Sign In failed") }
            }
        }
    }
    ...
    fun registerDevice(deviceId: String, displayName: String, manualCode: String, onResult: (Boolean) -> Unit = {}) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            runCatching {
                ...
                _uiState.update { it.copy(statusMessage = "Device registered successfully") }
                onResult(true)
            }.onFailure { error ->
    ...
    fun addSensorRule(deviceId: String, name: String, sensorType: String, minValue: Double?, maxValue: Double?) {
        viewModelScope.launch(Dispatchers.IO) {
            runCatching {
                api.addSensorRule(deviceId, sensorType, minValue, maxValue, name)
                refreshOnce()
            }.onSuccess {
                setStatus("Sensor rule added")
            }.onFailure { error ->
                setStatus(error.message ?: "Failed to add sensor rule")
            }
        }
    }

    fun removeSensorRule(id: String) {
        viewModelScope.launch(Dispatchers.IO) {
            runCatching {
                api.removeSensorRule(id)
                refreshOnce()
            }.onSuccess {
                setStatus("Sensor rule removed")
            }.onFailure { error ->
                setStatus(error.message ?: "Failed to remove sensor rule")
            }
        }
    }
    ...
    private suspend fun refreshOnce() {
        runCatching {
            ...
            val sensorReadings = api.fetchSensorReadings()
            ...
            _uiState.update {
                it.copy(
                    ...
                    sensorRules = bootstrap.sensorRules,
